...
TEST_CASE("decode_bpf refuses an unaligned buffer", "[seccomp]") {
    auto r = abcpwn::commands::seccomp::decode_bpf({reinterpret_cast<const std::uint8_t*>("abc"), 3});
    REQUIRE_FALSE(r);
...
TEST_CASE("SeccompCommand disasm action runs end-to-end", "[seccomp][command]") {
    ...
    cmd.input  =
        "20000000" "00000000"   // A = sys_number
        "15000100" "3b000000"   // if (A == 0x3b) ALLOW
        "06000000" "0000ff7f"   // return ALLOW
        "06000000" "00000000";  // return KILL
    auto r = cmd.run(ctx);
    REQUIRE(r);
...
TEST_CASE("libc id round-trips a known glibc 2.34 fingerprint", "[libc][command]") {
    ...
    for (const auto& s : r->sections) {
        for (const auto& f : s.findings) {
            if (f.detail.find("libc6_2.34") != std::string::npos) found = true;
        }
    }
    REQUIRE(found);
}
...
TEST_CASE("libc download surfaces FeatureDisabled when build lacks network",
          "[libc][command]")
{
...
TEST_CASE("libc search filters in-binary table by substring", "[libc][command]") {
    ...
    for (const auto& s : r->sections) {
        for (const auto& f : s.findings) {
            if (f.title.find("2.39") != std::string::npos) any_2_39 = true;
        }
    }
    REQUIRE(any_2_39);
}
