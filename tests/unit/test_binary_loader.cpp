...
TEST_CASE("sniff_format recognizes ELF, PE, and Mach-O magic", "[binary_loader]") {
    ...
    REQUIRE(sniff_format(std::string_view("\x7f""ELF__", 6)) == BinaryFormat::Elf);
...
TEST_CASE("load on a hardened ELF surfaces format, arch, entry, and danger", "[binary_loader][fixture]") {
