...
TEST_CASE("find_gadgets discovers a simple ret-only chain", "[rop][gadget]") {
    ...
    sec.bytes = {
        0x5f, 0xc3,             // 0x400000: pop rdi ; ret
        0x5e, 0xc3,             // 0x400002: pop rsi ; ret
        0x0f, 0x05,             // 0x400004: syscall (terminates differently)
    ...
    bool has_ret = false;
    for (const auto& g : *r) {
        if (g.text == "pop rdi ; ret") has_pop_rdi = true;
        if (g.text == "pop rsi ; ret") has_pop_rsi = true;
        if (g.text == "ret")           has_ret = true;
    }
    REQUIRE(has_pop_rdi);
    REQUIRE(has_pop_rsi);
    REQUIRE(has_ret);
...
TEST_CASE("find_gadgets respects bad-chars filter", "[rop][gadget]") {
    ...
    bool has_pop_rdi = false;
    for (const auto& g : *r) {
        if (g.text == "pop rax ; ret") has_pop_rax = true;
        if (g.text == "pop rdi ; ret") has_pop_rdi = true;
    }
...
TEST_CASE("find_gadgets results sort ascending by address", "[rop][gadget]") {
    ...
    sec.bytes = {
        0x5e, 0xc3,             // 0x1000: pop rsi ; ret
        0x5f, 0xc3,             // 0x1002: pop rdi ; ret
        0x58, 0xc3,             // 0x1004: pop rax ; ret
    };
    GadgetSearchOptions opts;
    opts.arch       = Arch::X86_64;
    opts.terminator = Terminator::Ret;
...
TEST_CASE("GadgetCommand finds gadgets in hello-hardened fixture", "[rop][command]") {
    ...
    for (const auto& line : r->raw_lines) {
        if (line.find("ret") != std::string::npos) { any_ret = true; break; }
    }
    REQUIRE(any_ret);
}
...
TEST_CASE("OneGadgetCommand returns command result (may be empty)", "[rop][command]") {
    ...
    for (const auto& f : r->sections[0].findings) {
        if (f.title.find("no /bin/sh") != std::string::npos) saw_no_binsh = true;
    }
