...
[[nodiscard]] core::Result<std::vector<BpfInsn>> decode_bpf(std::span<const std::uint8_t> bytes);
...
disassemble_bpf(std::span<const BpfInsn> insns, arch::Arch arch_for_syscalls = arch::Arch::X86_64);
