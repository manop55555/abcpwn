...
[[nodiscard]] std::optional<std::size_t> find_marker_offset(
...
[[nodiscard]] std::string build_leak_payload(
...
[[nodiscard]] core::Result<std::vector<std::uint8_t>> build_write_payload(
    const WriteSpec& spec);

class FmtCommand : public core::ICommand {
public:
    [[nodiscard]] std::string_view name()        const noexcept override { return "fmt"; }
