...
spdlog::level::level_enum level_from_verbosity(int v) {
    if (v <= -2) return spdlog::level::err;
    if (v == -1) return spdlog::level::warn;
    if (v == 0)  return spdlog::level::info;
    if (v == 1)  return spdlog::level::debug;
...
std::shared_ptr<spdlog::logger> get_logger() {
    ...
    if (!g_logger) {
        ...
        g_logger = std::make_shared<spdlog::logger>(
            kLoggerName, spdlog::sinks_init_list{console});
