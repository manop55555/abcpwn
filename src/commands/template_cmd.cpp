...
std::string render_template(std::string_view strategy, std::string_view binary_path) {
    ...
    if (strategy == "ret2win") {
        ...
        os << "# strategy: SROP via abcpwn srop --syscall N --syscall-arg ARG\n"
        ...
        os << "# strategy: format-string leak\n"
...
void TemplateCommand::setup(CLI::App& app) {
    app.add_option("strategy", strategy, "ret2win | ret2libc | rop | srop | fmt-leak | heap")
        ->required();
    app.add_option("binary", binary_path, "Challenge binary path")->required();
    app.add_option("-o,--output", output_path, "Write the rendered template to this path");
}
...
core::Result<core::CommandResult> TemplateCommand::run(const core::Context& /*ctx*/) {
    ...
    if (!output_path.empty()) {
        ...
        res.raw_lines.push_back("wrote " + std::to_string(rendered.size()) + " bytes to "
                                + output_path);
        return res;
    }
    core::CommandResult res;
