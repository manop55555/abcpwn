...
void DynelfCommand::setup(CLI::App& app) {
    app.add_option("--leak", leaks,
        "<addr>=<hex-bytes> (repeatable)");
    app.add_option("--libc-db", libc_db_path,
        "Local libc database root (not used in v0.1)");
}
...
core::Result<core::CommandResult> DynelfCommand::run(const core::Context& /*ctx*/) {
    ...
    for (const auto& spec : leaks) {
        ...
        core::Finding f(core::Severity::Info,
            std::string(addr_buf),
            std::to_string(p->bytes.size()) + " bytes: "
                + encoding::hex_encode(p->bytes, " "));
