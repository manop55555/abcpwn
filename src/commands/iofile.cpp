...
[[nodiscard]] std::optional<Technique> technique_from_string(std::string_view s) noexcept {
    if (s == "fsop-leak")        return Technique::FsopLeak;
    if (s == "fsop-exec")        return Technique::FsopExec;
    if (s == "vtable-overwrite") return Technique::VtableOverwrite;
    return std::nullopt;
}

[[nodiscard]] std::string_view technique_name(Technique t) noexcept {
    switch (t) {
        case Technique::FsopLeak:        return "fsop-leak";
        case Technique::FsopExec:        return "fsop-exec";
        case Technique::VtableOverwrite: return "vtable-overwrite";
    }
    return "unknown";
}
...
void IofileCommand::setup(CLI::App& app) {
    app.add_option("technique", technique_str,
        "fsop-leak | fsop-exec | vtable-overwrite")->required();
    app.add_option("--libc-version", libc_version_str,
        "Target libc version (default 2.34)");
}
...
core::Result<core::CommandResult> IofileCommand::run(const core::Context& /*ctx*/) {
    ...
    layout_sec.findings.emplace_back(core::Severity::Info, "write_base",   hex(layout.write_base_offset));
    layout_sec.findings.emplace_back(core::Severity::Info, "write_ptr",    hex(layout.write_ptr_offset));
    layout_sec.findings.emplace_back(core::Severity::Info, "write_end",    hex(layout.write_end_offset));
    ...
    layout_sec.findings.emplace_back(core::Severity::Info, "wide_data",    hex(layout.wide_data_offset));
    ...
    switch (*t) {
        case Technique::FsopLeak:
            tech_sec.findings.emplace_back(core::Severity::Info, "primitive",
            ...
            tech_sec.findings.emplace_back(core::Severity::Medium, "applicability",
        ...
        case Technique::FsopExec:
            tech_sec.findings.emplace_back(core::Severity::Info, "primitive",
                ...
                "forged FILE pointer.");
            tech_sec.findings.emplace_back(core::Severity::High, "applicability",
        ...
        case Technique::VtableOverwrite:
            tech_sec.findings.emplace_back(core::Severity::Info, "primitive",
                ...
                "at " + hex(layout.vtable_offset) + " offset. Simplest "
