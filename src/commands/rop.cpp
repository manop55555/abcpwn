...
[[nodiscard]] std::optional<rop::Gadget> find_gadget_text_eq(
    std::span<const rop::Gadget> gadgets, std::string_view text) noexcept
{
...
void RopCommand::setup(CLI::App& app) {
    ...
    app.add_option("--syscall", syscall_number,
        "Build a syscall chain with this number");
    app.add_option("--syscall-arg", syscall_args,
        "Repeat per arg: --syscall 59 --syscall-arg 0x600000 --syscall-arg 0 ...");
    app.add_option("--ret2win", ret2win,
        "Symbol name to jump to (currently unsupported)");
    app.add_option("--leak", leak_symbol,
        "Symbol whose address to leak (currently unsupported)");
...
core::Result<core::CommandResult> RopCommand::run(const core::Context& /*ctx*/) {
    ...
    if (sections.empty()) {
        return core::err(core::ErrorCode::NotFound,
            "rop: no executable sections in target");
    }
    ...
    if (!syscall_g) syscall_g = find_gadget_text_eq(*gadgets, "syscall ; ret");

    std::vector<std::string> missing;
    if (!pop_rax)   missing.emplace_back("pop rax ; ret");
    if (!syscall_g) missing.emplace_back("syscall");
    if (!pop_rdi && !syscall_args.empty())             missing.emplace_back("pop rdi ; ret");
    if (!pop_rsi && syscall_args.size() >= 2)          missing.emplace_back("pop rsi ; ret");
    if (!pop_rdx && syscall_args.size() >= 3)          missing.emplace_back("pop rdx ; ret");
    if (!missing.empty()) {
        std::string m;
    ...
    auto add_pair = [&](std::string_view what, std::uint64_t addr,
    ...
    add_pair("pop rax ; ret",
        pop_rax->address, static_cast<std::uint64_t>(syscall_number));
