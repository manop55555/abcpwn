...
void SropCommand::setup(CLI::App& app) {
    ...
    app.add_option(
        "--syscall", syscall_number, "Build a syscall sigframe with this syscall number in rax");
    app.add_option("--syscall-arg", syscall_args, "Append per arg: --syscall-arg <value>");
...
core::Result<core::CommandResult> SropCommand::run(const core::Context& /*ctx*/) {
    ...
    if (syscall_number >= 0) {
        ...
        if (!syscall_args.empty())
            f.rdi = syscall_args[0];
        if (syscall_args.size() >= 2)
            f.rsi = syscall_args[1];
        if (syscall_args.size() >= 3)
            f.rdx = syscall_args[2];
        if (syscall_args.size() >= 4)
            f.r10 = syscall_args[3];
        if (syscall_args.size() >= 5)
            f.r8 = syscall_args[4];
        if (syscall_args.size() >= 6)
            f.r9 = syscall_args[5];
    }
    f.rip = rip;
    f.rsp = rsp;
