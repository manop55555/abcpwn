...
void SafeLinkCommand::setup(CLI::App& app) {
    app.add_option("value", ptr_or_encoded, "Pointer (when encoding) or encoded fd (when decoding)")
        ->required();
    app.add_option("pos", pos, "Position the value lives at (the chunk address holding the fd)")
        ->required();
    app.add_flag("--decode{false},--encode{true}",
                 encode,
...
core::Result<core::CommandResult> SafeLinkCommand::run(const core::Context& /*ctx*/) {
    ...
    const auto out =
        encode ? safe_link_encode(ptr_or_encoded, pos) : safe_link_decode(ptr_or_encoded, pos);
    char buf[32];
