...
[[nodiscard]] core::Result<DslStep> parse_line(std::string_view raw, std::size_t line_no) {
    ...
    const auto rest = first_space == std::string_view::npos
        ? std::string_view{}
    ...
    if (opname == "recvuntil") { step.op = DslOp::RecvUntil; return needs_arg(); }
    if (opname == "recvline")  { step.op = DslOp::RecvLine;  return step; }
    if (opname == "send")      { step.op = DslOp::Send;      return needs_arg(); }
    if (opname == "sendline")  { step.op = DslOp::SendLine;  return needs_arg(); }
    if (opname == "expect")    { step.op = DslOp::Expect;    return needs_arg(); }
    if (opname == "recvn" || opname == "sleep") {
        if (rest.empty()) {
            return core::err(core::ErrorCode::InvalidInput,
                "pwn dsl: '" + std::string(opname)
                + "' requires a numeric argument (line "
                + std::to_string(line_no) + ")");
        }
        std::int64_t v = 0;
    ...
    if (opname == "set") {
        ...
        if (sp == std::string_view::npos) {
            return core::err(core::ErrorCode::InvalidInput,
                "pwn dsl: 'set' wants <var> <value> (line "
                + std::to_string(line_no) + ")");
        }
        step.op   = DslOp::Set;
...
core::Result<std::vector<DslStep>> parse_dsl(std::string_view source) {
    ...
    while (pos <= source.size()) {
        ...
        const auto line = source.substr(pos, nl == std::string_view::npos
...
void PwnCommand::setup(CLI::App& app) {
    app.add_option("target", target,
        "host:port | unix:/path | ./local-binary")->required();
...
core::Result<core::CommandResult> PwnCommand::run(const core::Context& /*ctx*/) {
    ...
    sec.findings.emplace_back(core::Severity::Info, "tube",
        spec->kind == TubeKind::Tcp        ? std::string("TCP ")    + spec->host_or_path + ":" + std::to_string(spec->port)
        : spec->kind == TubeKind::UnixSocket ? std::string("Unix ") + spec->host_or_path
                                             : std::string("Process ") + spec->host_or_path);
    sec.findings.emplace_back(core::Severity::Info, "timeout",
    ...
    if (!steps.empty()) {
        ...
        for (std::size_t i = 0; i < steps.size(); ++i) {
            ...
            std::string label;
            switch (s.op) {
                case DslOp::RecvUntil: label = "recvuntil"; break;
                case DslOp::RecvLine:  label = "recvline";  break;
                case DslOp::RecvN:     label = "recvn " + std::to_string(s.numeric); break;
                case DslOp::Send:      label = "send";      break;
                case DslOp::SendLine:  label = "sendline";  break;
                case DslOp::Sleep:     label = "sleep " + std::to_string(s.numeric) + "ms"; break;
                case DslOp::Expect:    label = "expect (regex)"; break;
                case DslOp::Set:       label = "set " + s.arg1 + "=" + s.arg2; break;
            }
