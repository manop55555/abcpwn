...
void PwninitCommand::setup(CLI::App& app) {
    ...
    app.add_option("--template", template_strategy,
        "Strategy passed to the template command (ret2libc, rop, ...)");
    app.add_flag("--no-patch", no_patch,
        "Skip the patchelf step (just plan and stop)");
}
...
core::Result<core::CommandResult> PwninitCommand::run(const core::Context& ctx) {
    ...
    if (bin_path.empty() || libc_path.empty() || ld_path.empty()) {
        ...
        for (const auto& ent : std::filesystem::directory_iterator(directory, ec)) {
            ...
            if (libc_path.empty() && (ends_with(name, ".so.6") || name.find("libc") != std::string::npos)) {
                libc_path = p;
            } else if (ld_path.empty() && name.find("ld-linux") != std::string::npos) {
                ld_path = p;
            } else if (bin_path.empty() && (ent.status().permissions() & std::filesystem::perms::owner_exec)
    ...
    plan.findings.emplace_back(core::Severity::Info, "binary",
        bin_path.empty() ? "(not found in directory)" : bin_path);
    plan.findings.emplace_back(core::Severity::Info, "libc",
        libc_path.empty() ? "(not found in directory)" : libc_path);
    plan.findings.emplace_back(core::Severity::Info, "ld-linux",
        ld_path.empty() ? "(not found; would be downloaded if network)" : ld_path);
    ...
    if (ld_path.empty()) {
        const std::string detail = network_compiled_in() && ctx.allow_network
        ...
        steps.findings.emplace_back(core::Severity::Info,
            "2. fetch ld-linux", detail);
    } else {
        steps.findings.emplace_back(core::Severity::Info,
            "2. fetch ld-linux", "ld already present, skip");
    }
    if (no_patch) {
        steps.findings.emplace_back(core::Severity::Info,
            "3. patchelf", "skipped via --no-patch");
    } else {
        steps.findings.emplace_back(core::Severity::Info,
            "3. patchelf",
    ...
    if (!network_compiled_in() || !ctx.allow_network) {
        steps.findings.emplace_back(core::Severity::Medium, "note",
