...
std::vector<ByteDelta> byte_diff(
...
void DiffCommand::setup(CLI::App& app) {
    ...
    app.add_option("--max-results", max_results,
        "Cap on number of byte deltas reported (default 1024)");
}
...
core::Result<core::CommandResult> DiffCommand::run(const core::Context& ctx) {
    ...
    if (!a) return core::err(a.error());
    auto b = core::safe_io::read_file(file_b, opts);
    if (!b) return core::err(b.error());

    std::vector<std::uint8_t> av(a->size()), bv(b->size());
    for (std::size_t i = 0; i < a->size(); ++i) av[i] = static_cast<std::uint8_t>((*a)[i]);
    for (std::size_t i = 0; i < b->size(); ++i) bv[i] = static_cast<std::uint8_t>((*b)[i]);

    const auto deltas = byte_diff(av, bv);
    core::CommandResult res;
    auto& sec = res.sections.emplace_back();
    sec.title = "byte diff (" + std::to_string(av.size()) + "/" + std::to_string(bv.size()) + " bytes)";
    std::size_t shown = 0;
    for (const auto& d : deltas) {
        ...
        sec.findings.emplace_back(core::Severity::Info, hex_str(d.offset),
...
void PatchCommand::setup(CLI::App& app) {
    ...
    app.add_option("--byte", byte_patches,
        "Repeatable: offset=hex (e.g., 0x100=90909090)");
    app.add_option("--nop", nop_patches,
        "Repeatable: offset:count (NOP out N bytes from offset)");
    app.add_flag("--in-place", in_place,
        "Patch the original in place (default: writes .patched)");
    app.add_flag("--backup{true},--no-backup{false}", backup,
...
core::Result<core::CommandResult> PatchCommand::run(const core::Context& ctx) {
    ...
    if (!raw) return core::err(raw.error());
    std::vector<std::uint8_t> bytes(raw->size());
    for (std::size_t i = 0; i < raw->size(); ++i) {
        bytes[i] = static_cast<std::uint8_t>((*raw)[i]);
    }

    auto apply_byte = [&](std::uint64_t off, const std::vector<std::uint8_t>& payload)
        -> core::Result<void>
    {
        if (off + payload.size() > bytes.size()) {
            return core::err(core::ErrorCode::InvalidInput,
                "patch: byte patch beyond end of file");
        }
    ...
    const std::string out_path = in_place
        ? target
        : target + ".patched";
    ...
    if (auto w = core::safe_io::write_file_atomic(
        ...
        std::span<const std::byte>(
            reinterpret_cast<const std::byte*>(bytes.data()), bytes.size())); !w)
    {
        return core::err(w.error());
    }

    core::CommandResult res;
    res.raw_lines.push_back("wrote " + std::to_string(bytes.size())
        + " bytes to " + out_path);
