...
std::optional<Technique> technique_from_string(std::string_view s) noexcept {
    if (s == "tcache-poison")
        return Technique::TcachePoison;
    if (s == "fastbin")
        return Technique::Fastbin;
    if (s == "house-of-force")
        return Technique::HouseOfForce;
    if (s == "house-of-orange")
        return Technique::HouseOfOrange;
    if (s == "house-of-apple")
        return Technique::HouseOfApple;
    if (s == "unsorted-bin")
        return Technique::UnsortedBin;
    return std::nullopt;
}

std::optional<LibcEra> libc_era_from_string(std::string_view s) noexcept {
    if (s == "2.23")
        return LibcEra::V2_23;
    if (s == "2.27")
        return LibcEra::V2_27;
    if (s == "2.31")
        return LibcEra::V2_31;
    if (s == "2.34")
        return LibcEra::V2_34;
    if (s == "2.39")
        return LibcEra::V2_39;
    return std::nullopt;
}

std::string_view technique_name(Technique t) noexcept {
    switch (t) {
    case Technique::TcachePoison:
        return "tcache-poison";
    case Technique::Fastbin:
        return "fastbin";
    case Technique::HouseOfForce:
        return "house-of-force";
    case Technique::HouseOfOrange:
        return "house-of-orange";
    case Technique::HouseOfApple:
        return "house-of-apple";
    case Technique::UnsortedBin:
        return "unsorted-bin";
    }
    return "unknown";
}
...
constexpr std::array<CompatEntry, 30> kMatrix = {{
    ...
    {Technique::TcachePoison, LibcEra::V2_23, Status::Broken, "tcache not present before 2.26"},
    {Technique::TcachePoison,
     LibcEra::V2_27,
     Status::Works,
    ...
    {Technique::HouseOfForce, LibcEra::V2_34, Status::Broken, "permanently mitigated"},
    {Technique::HouseOfForce, LibcEra::V2_39, Status::Broken, "permanently mitigated"},

    // House of Orange: chain unsorted-bin attack into FILE struct
    // exploitation via _IO_list_all. Works on older glibcs.
    {Technique::HouseOfOrange,
     LibcEra::V2_23,
     Status::Works,
    ...
    {Technique::UnsortedBin, LibcEra::V2_23, Status::Works, "classic unsorted-bin attack"},
    {Technique::UnsortedBin, LibcEra::V2_27, Status::Works, "classic unsorted-bin attack"},
    {Technique::UnsortedBin,
     LibcEra::V2_31,
     Status::Mitigated,
...
std::string_view technique_description(Technique t) noexcept {
    switch (t) {
    case Technique::TcachePoison:
        return "Free a chunk twice (or overlap free chunks). Overwrite the "
               ...
               "required.";
    case Technique::Fastbin:
        return "Overflow a free fastbin chunk's fd pointer. The next alloc "
    ...
    case Technique::HouseOfForce:
        return "Overflow the top chunk's size to a huge value. A "
    ...
    case Technique::HouseOfOrange:
        return "Overflow the top-chunk size below page-rounded boundary; "
    ...
    case Technique::HouseOfApple:
        return "FILE-stream technique targeting _IO_wfile_jumps + "
    ...
    case Technique::UnsortedBin:
        return "Overwrite the bk pointer of a freed unsorted-bin chunk to "
...
void HeapCommand::setup(CLI::App& app) {
    app.add_option("technique",
                   technique_str,
                   "tcache-poison | fastbin | house-of-force | house-of-orange | "
                   "house-of-apple | unsorted-bin")
        ->required();
    app.add_option("--libc-version",
                   libc_version_str,
                   "Target libc version (2.23, 2.27, 2.31, 2.34, 2.39). Default 2.34.");
    app.add_option(
        "--target-address", target_address, "Target write address (hex) for the chosen technique");
    app.add_flag(
        "--all-libcs", all_libcs, "Report compatibility across every libc era in the matrix");
}
...
core::Result<core::CommandResult> HeapCommand::run(const core::Context& /*ctx*/) {
    ...
    if (all_libcs) {
        ...
        for (const auto& row : compatibility_matrix()) {
            if (row.technique != *tech)
                continue;
            const auto sev = row.status == Status::Works       ? core::Severity::Info
    ...
    {
        auto& sec = res.sections.emplace_back();
        sec.title = std::string(technique_name(*tech)) + " against glibc "
                    + std::string(libc_era_label(*era));
        const auto sev = row.status == Status::Works       ? core::Severity::Info
                         : row.status == Status::Mitigated ? core::Severity::Medium
                                                           : core::Severity::High;
        sec.findings.emplace_back(sev, "status", std::string(status_label(row.status)));
        sec.findings.emplace_back(core::Severity::Info, "rationale", std::string(row.rationale));
    }
    {
        auto& sec = res.sections.emplace_back();
        sec.title = "technique";
        sec.findings.emplace_back(
            core::Severity::Info, "summary", std::string(technique_description(*tech)));
        if (target_address != 0) {
            sec.findings.emplace_back(core::Severity::Info, "target", hex_str(target_address));
        }
    }
