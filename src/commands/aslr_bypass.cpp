...
void AslrBypassCommand::setup(CLI::App& app) {
    app.add_flag(
        "--partial-overwrite", partial_overwrite, "Describe 1/2-byte partial-overwrite technique");
    app.add_flag("--brute-force", brute_force, "Report expected brute-force attempts");
    app.add_flag("--canary-leak", canary_leak, "Print a canary-leak template");
    app.add_option("--entropy-bits",
                   entropy_bits,
...
core::Result<core::CommandResult> AslrBypassCommand::run(const core::Context& /*ctx*/) {
    ...
    if (brute_force) {
        const std::uint64_t attempts =
            (entropy_bits >= 64) ? std::uint64_t(-1) : (std::uint64_t(1) << entropy_bits);
        sec.findings.emplace_back(
            core::Severity::Info, "entropy", std::to_string(entropy_bits) + " bits");
        sec.findings.emplace_back(core::Severity::Info,
                                  "expected attempts",
                                  std::to_string(attempts / 2)); // mean of geometric distribution
        sec.findings.emplace_back(core::Severity::Info, "worst case", std::to_string(attempts));
