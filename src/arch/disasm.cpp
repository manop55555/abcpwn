...
CsConfig pick(const DisasmOptions& opts) {
    CsConfig c;
    const auto endian_flag = (opts.endian == Endian::Big)
    ...
    switch (opts.arch) {
        ...
        case Arch::Arm:
            c.arch = CS_ARCH_ARM;
            c.mode = static_cast<cs_mode>(
                (opts.thumb_mode ? CS_MODE_THUMB : CS_MODE_ARM) | endian_flag);
            break;
        case Arch::Aarch64:
...
core::Result<std::vector<Instruction>> disassemble(
